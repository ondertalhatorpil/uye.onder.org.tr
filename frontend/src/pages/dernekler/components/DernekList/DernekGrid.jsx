...
const DernekGrid = ({ dernekler, viewMode, filters, onClearFilters }) => {
  ...
  const DernekCard = ({ dernek }) => (
    <div className={`bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden group hover:scale-105 ${
      ...
      {/* Logo Bölümü */}
      <div className={`${viewMode === 'list' ? 'flex-shrink-0 w-32' : ''} relative`}>
        <div className={`${viewMode === 'list' ? 'h-32' : 'h-48'} bg-gray-100 overflow-hidden`}>
      ...
      <div className={`p-6 ${viewMode === 'list' ? 'flex-1 flex flex-col justify-between' : ''}`}>
          ...
          <div className={viewMode === 'list' ? 'flex-1 pr-4' : ''}>
            ...
            <div className="space-y-2 mb-4">
              ...
              <div className="flex items-center text-gray-600">
                ...
                <span className="text-sm font-medium">
                  <span className="text-red-600 font-semibold">{dernek.uye_sayisi || 0}</span> üye
                </span>
              </div>
            </div>
          </div>
          ...
          {viewMode === 'list' && (
            <div className="flex flex-col space-y-3">
              <Link
                ...
                className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition-all hover:scale-105"
              >
              ...
              <Link
                ...
                className="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-all hover:scale-105"
              >
        ...
        {viewMode === 'grid' && (
          <div className="flex space-x-3 mt-4">
            <Link
              to={`/dernekler/${encodeURIComponent(dernek.dernek_adi)}`}
              className="flex-1 inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition-all hover:scale-105"
            >
              <FiExternalLink className="mr-2 h-4 w-4" />
              Görüntüle
            </Link>
            <Link
              to={`/uyeler?dernek=${encodeURIComponent(dernek.dernek_adi)}`}
              className="flex-1 inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-all hover:scale-105"
            >
              <FiUsers className="mr-2 h-4 w-4" />
              Üyeler
            </Link>
